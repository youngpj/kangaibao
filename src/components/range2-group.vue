<template>
    <div class="range-group">
        ...
        <div class="label" v-for="(item, index) in params" :key="index" :style="{ 'left': (kedu * index) + '%' }" :class="{'active': checkIndex == index && checkIndex != 0, 'forbid': (index < minValue) || (forbid && index > 0)}">{{item.label}}<span>万</span></div>
    </div>
</template>
<script>
export default {
    ...
    props: ['params', 'value', 'minValue', 'forbid'],
    methods: {
        onRange(evt) {
            if (this.forbid) return;
            var value = evt.target.value;
            if (value < this.minValue * this.kedu) return;
