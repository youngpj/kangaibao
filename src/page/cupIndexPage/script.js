...
import cupSwitch from "@/components/cup-switch.vue";
import cupRadio from "@/components/cup-radio.vue";
...
export default {
    ...
    Data: {
        ...
        range1Upgrade: true,
        range2Upgrade: true,
        randcheck: false, //普通门急诊选项
        hospitalChoose: 3, //住院医疗保障
        unfold: false,
        moneyChange: false,
    ...
    methods: {
                    ...
                    this.$http.post('/api/facade/policy/pricing', {
                        ...
                        if (res.data.code === '0000') {
                            ...
                            this.$localStorage.setItem('storeInfos', {
                                'MPN':  parseInt(this.hospitalChoose),
                                'OPN':  this.range1,
                                'SOCIALSECURITY': this.userCheckInfo.hasShebao == '有' ? 1 : 0,
        ...
        toInsure() {
            ...
            this.$hmt.trackEvent2('bwb', 'to_order');
        },
        checkUserInfo() {
            console.log(this.inconformityAge)
        ...
        switchToPage(url) {
            v.$hmt.trackEvent(this.version, 'click', 'home_keb')
            this.$router.push("/")
        ...
        unfoldDetail() {
            this.unfold = true;
            v.$hmt.trackEvent(this.version, 'click', 'home_more')
        },
        ...
        showImgViewer: function(viewer) {
            if (viewer == 'xuzhiViewer') {
                v.$hmt.trackEvent(this.version, 'click', 'home_notice')
            }
            if (viewer == 'tiaokuanViewer') {
                v.$hmt.trackEvent(this.version, 'click', 'home_terms')
            }
        ...
        changeSwitch(val) {
            console.log(val);
            this.randcheck = val;
            v.$hmt.trackEvent(this.version, 'click', 'home_emergency')
        },
        chooseHospital(val) {
            this.hospitalChoose = val;
            v.$hmt.trackEvent(this.version, 'click', 'home_hospital')
        },
        //默认保费值
        defaultPayment() {
            //不带门急诊，30万默认394，12万默认329
            //带门急诊，30万默认733，12万默认668
            if (this.hospitalChoose == '3') {
                this.totalPayment = this.randcheck ? 733 : 394
            }
            if (this.hospitalChoose == '2') {
                this.totalPayment = this.randcheck ? 668 : 329
            }
            console.log(this.randcheck)
            console.log(this.hospitalChoose)
        }
    },
    computed: {
    ...
    watch: {
        ...
        'hospitalChoose': function(value, oldValue) {
            this.range2 = value
            if (this.userCheckInfo.birthDate) {
                this.getTotalPayment()
            } else {
